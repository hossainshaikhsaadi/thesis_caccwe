...
def align_bert_multiple(train, model1, model2, 
    ...
    trainer = torch.optim.Adam([param for param in model1.parameters() if
    ...
    model1.eval() # freeze and remember initial model
    model2.eval()
    total_processed = 0
        ...
        for i in range(0, num_sentences, batch_size):
            ...
            for j, language in enumerate(languages):
                ...
                for k in range(0, len(ss_1), splitbatch_size):
                    ...
                    ann_1, ann_2 = model1(s_1), model2(s_2)
                    ones = torch.tensor([1] * ann_1[idx_1].shape[0])

                    loss_1 = F.mse_loss(ann_1[idx_1], ann_2[idx_2])
                    loss_2 = F.cosine_embedding_loss(ann_1[idx_1], ann_2[idx_2], ones)
...
def evaluate_retrieval_context(dev, model1, model2):
    sent_1, sent_2, align = dev
    idx_1, idx_2 = pick_aligned(sent_1, sent_2, align)
    model1.eval()
    model2.eval()
    with torch.no_grad():
        ann_1 = model1(sent_1)[idx_1].detach().cpu().numpy()
        ann_2 = model2(sent_2)[idx_2].detach().cpu().numpy()
...
def evaluate_retrieval_noncontext(dev, model1, model2):
    sent_1, sent_2, align = dev
    idx_1, idx_2 = pick_aligned(sent_1, sent_2, align)
    all_words=[]
    for j in sent_1:
...
model_type = 'bert'
src_model_name = 'dbmdz/bert-base-german-cased'
src_lower_case = False
tgt_model_name = 'bert-base-cased'
single_layer_type = 'transformer'
tgt_lower_case = False
save_directory = '/home/hsaadi/helper_folder/saved_models/'
...
model1 = WordLevelBert(src_model_name, src_lower_case, model_type, use_saved=False)
model2 = WordLevelBert(tgt_model_name, tgt_lower_case, model_type, use_saved=False)

train = [(sent_1[num_test+num_dev:], sent_2[num_test+num_dev:], align[num_test+num_dev:]) for sent_1, sent_2, align in data]

batch_size = 8 #their default was given 8 please change it i used 4 because of memory issue
epochs = 1 #their default was given 1
learning_rate=0.000005 #their default was given 0.00005
align_bert_multiple(train, model1, model2, num_sent, languages, batch_size, learning_rate=learning_rate, epochs=epochs)
