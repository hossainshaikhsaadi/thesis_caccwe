...
def align_bert_multiple(train, single_model, model1, model2, 
        ...
        for i in range(0, num_sentences, batch_size):
            ...
            for j, language in enumerate(languages):
                ...
                for k in range(0, len(ss_1), splitbatch_size):
                    ...
                    ann_1, ann_2_base = model1(s_1), model2(s_2)
...
def evaluate_retrieval_context(dev, model1, model2, single_model):
     with tqdm(total=6) as pbar:
        ...
        model2.eval()
        with torch.no_grad():
            ann_1 = model1(sent_1)[idx_1]
            ann_1 = ann_1.unsqueeze(0)
            ann_1 = single_model(ann_1)
            ann_1 = ann_1.squeeze(0)
            #ann_1 = ann_1[idx_1].detach().cpu().numpy()
            ann_2 = model2(sent_2)[idx_2]
...
def evaluate_retrieval_noncontext(dev, model1, model2, single_model):
    sent_1, sent_2, align = dev
    idx_1, idx_2 = pick_aligned(sent_1, sent_2, align)
    all_words=[]
    for j in sent_1:
...
align_bert_multiple(train, single_layer, model1, model2, num_sent, languages, batch_size, learning_rate=learning_rate, epochs=epochs)
...
for lang, dev_lang in zip(languages, test):
    ...
    print("Word retrieval accuracy:", evaluate_retrieval_context(dev_lang, model1, model2, single_layer))
'''
print('\nNon Context Evaluation: \n')
for lang, dev_lang in zip(languages, test):
    print("For "+str(lang)+"-en  : ")
    print("Word retrieval accuracy:", evaluate_retrieval_noncontext(dev_lang, model1, model2, single_layer))
'''