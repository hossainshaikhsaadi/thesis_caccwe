...
from transformers.adapters.configuration import PfeifferConfig,  HoulsbyConfig
import warnings
...
def get_bert(bert_model, bert_do_lower_case, model_type, use_saved=False):
    ...
    if use_saved == True:
        model.load_adapter(adapter_save_path)
...
def align_bert_multiple(train, model, single_model,
    ...
    model_trainer = torch.optim.Adam([param for param in model.parameters() if
                                param.requires_grad], lr=learning_rate, 
                               betas=(0.9, 0.99), eps=1e-9)

    single_model_trainer = torch.optim.Adam([param for param in single_model.parameters() if
                                param.requires_grad], lr=learning_rate, 
                               betas=(0.9, 0.99), eps=1e-9)
                               
    # set up functions to do linear lr warmup
    def set_lr(new_lr):
        for param_group in model_trainer.param_groups:
            param_group['lr'] = new_lr
        for param_group in single_model_trainer.param_groups:
            param_group['lr'] = new_lr
    ...
    for epoch in range(epochs):
        for i in range(0, num_sentences, batch_size):
            ...
            loss.backward()
            model_trainer.step()
            single_model_trainer.step()
            model_trainer.zero_grad()
            single_model_trainer.zero_grad()
                
    #torch.save(model.state_dict(), adapter_save_path)
    model.bert.save_adapter(adapter_save_path, adapter_name)
    model_state_dict = single_model.state_dict()
    torch.save(model_state_dict, single_model_save_path)
...
def add_adapters(model):
    ...
    model.bert.add_adapter(adapter_name, config = adapter_config)
    model.bert.train_adapter(adapter_name)
    return model
...
prefix = 'dbert'
src_model_name = 'distilbert-base-multilingual-cased'
src_lower_case = False
base_directory = '/home/hsaadi/helper_folder/data/language_corpus/'
adapter_name = 'adapter_' + prefix + '_' + single_layer_type + '_' + 'adapter_bn-de-en'
adapters_path = '/home/hsaadi/helper_folder/saved_adapters/'
adapter_save_path = adapters_path + adapter_name
single_model_save_path = '/home/hsaadi/helper_folder/saved_models/' + prefix + '.' + single_layer_type + '.' + 'adapter.bn-de-en.pt'
...
learning_rate=0.0000001 #their default was given 0.00005
testing = True
training = True
...
single_layer.to('cuda')


if training == True:
    align_bert_multiple(train, model, single_layer, num_sent, languages, batch_size, learning_rate=learning_rate, epochs=epochs)
...
if testing == True:
    evaluation_data = [(sent_1[:num_test], sent_2[:num_test], align[:num_test]) for sent_1, sent_2, align in data]
else:
    evaluation_data = [(sent_1[num_test:num_test+num_dev], sent_2[num_test:num_test+num_dev], 
                        align[num_test:num_test+num_dev]) for sent_1, sent_2, align in data] 

use_cuda = False
model = WordLevelBert(src_model_name, src_lower_case, model_type, use_saved=True)
model.to('cuda')
if single_layer_type == 'linear':
    single_layer = Single_Linear(use_cuda = True)
else:
    single_layer = Single_Transformer_Encoder(use_cuda = True)
single_layer.to('cuda')
single_layer.load_state_dict(torch.load(single_model_save_path))
print('\nContext Evaluation: \n')
for lang, dev_lang in zip(languages, evaluation_data):
...
for lang, dev_lang in zip(languages, evaluation_data):
    print("For "+str(lang)+"-en  : ")
    print("Word retrieval accuracy:", evaluate_retrieval_noncontext(dev_lang, model, single_layer))

print('\nTime to train in minutes : '+str(minutes)+" \n")
