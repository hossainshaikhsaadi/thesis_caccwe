...
parser.add_argument("--output_src", type=str, default='', help="where to save aligned embeddings")
parser.add_argument("--output_tgt", type=str, default='', help="where to save aligned embeddings")


parser.add_argument("--knn", type=int, default=2, help="number of nearest neighbors in RCSL/CSLS")
...
if params.output_src != "":
    ...
    words_full_tgt, x_full_tgt = load_vectors(params.tgt_emb, maxload=-1, center=params.center, verbose=False)
    x = np.dot(x_full, R.T)
    x /= np.linalg.norm(x, axis=1)[:, np.newaxis] + 1e-8
    save_vectors(params.output_src, x, words_full)
    save_vectors(params.output_tgt, x_full_tgt, words_full_tgt)
    save_matrix(params.mapping_file + "-mat",  R)
